...
Early stages of election campaigns are frequently characterized by data
sparsity. In such low-information environments, partial pooling
struggles to identify group-level variances, often leading to complete
shrinkage or convergence failures. The scale parameters for \\\delta\\
and \\\tau\\ are anchored to keep the models robust and identifiable
throughout the entire cycle, transitioning gracefully from a
prior-dominated regime to a data-dominated one as the volume of polling
increases. Specific values for priors can be modified by the
[`configurar_prioris()`](https://rnmag.github.io/agregR/reference/configurar_prioris.md)
