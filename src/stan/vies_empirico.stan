...
//
// Este modelo utiliza vieses empíricos (históricos) como âncora para os
// parâmetros delta.
//
// alpha[t] ~ MultiNormal(alpha[t-1] + nu[t-1], Sigma_evo)
// delta[inst] ~ Normal(emp_delta_priori[inst], sd_delta_priori)
// mu[t] = softmax([alpha[t] + delta[inst], 0])
...
data {
  ...
  // Matriz de viés empírico [n_institutos, n_candidatos-1]
  matrix[n_institutos, n_candidatos-1] emp_delta_priori;
